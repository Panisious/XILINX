...
typedef struct {
    ...
    XTi_spi_if_uio_map maps[ MAX_UIO_MAPS ];
} XTi_spi_if_uio_info;

/***************** Variable Definitions **************************************/
static XTi_spi_if_uio_info uio_info;
...
static int uio_info_read_name(XTi_spi_if_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XTi_spi_if_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XTi_spi_if_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XTi_spi_if_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XTi_spi_if_Initialize(XTi_spi_if *InstancePtr, const char* InstanceName) {
	XTi_spi_if_uio_info *InfoPtr = &uio_info;
